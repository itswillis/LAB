...
class Country: 
    ...
    def print_births_table(self, start_year, end_year): 
        for region in sorted(self.regions_list): 
...
# Helper function - Display Births/Deaths statistics title
def display_statistics_title(title):
    print("*" * (len(title)))
    print(title)
    print("*" * (len(title)))

# Helper function - User inputs
...
# Helper function - Display Births/Deaths table
def display_births_deaths_table(country, start_year, end_year, event_type):
    title = "{} by Regions:".format(event_type)
    display_statistics_title(title)
    if event_type == "Births":
        country.print_births_table(start_year, end_year)
        total = country.get_total_births(start_year, end_year)
    elif event_type == "Deaths":
        country.print_deaths_table(start_year, end_year)
        total = country.get_total_deaths(start_year, end_year)

    total_line = "{} Total: {}".format(event_type, total)
    asterisks = "*" * len(total_line)
    print(asterisks)
    print(total_line)
    print(asterisks)
...
def main():
    ...
    display_statistics_title("Printing Births/Deaths Statistics")

    start_year = int(input("Enter the start year: "))
    end_year = int(input("Enter the end year: "))

    display_births_deaths_table(country, start_year, end_year, "Births")
    display_births_deaths_table(country, start_year, end_year, "Deaths")